...
if (fileinfo == -1) 
   error ("Script alex: Unable to open file name: %s, %s",infoshot, msg); 
endif;
...
if (exist("volt","var")==1)
  ...
  if (puntosv<=10)
    warning("Script alex: only %u points in voltage baseline",puntosv);
  endif;
...
if (exist("dint","var")==1)
  ...
  if (puntosi<=10)
    warning("Script alex: only %u points in intensity derivative baseline",puntosi);
  endif;
...
if (exist("phot","var")==1)
  ...
  if (puntosp<=10)
    warning("Script te: only %u points in photodiode baseline",puntosp);
  endif;
